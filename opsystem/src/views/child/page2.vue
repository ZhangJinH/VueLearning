    ...
    <div class="page2">
      ...
      <mu-table
        ...
        <mu-thead slot="header">
          <mu-tr>
            <mu-th tooltip="商品名">商品名</mu-th>
            <mu-th tooltip="销售地点">销售地点</mu-th>
            <mu-th  tooltip="销售时间">销售时间</mu-th>
            <mu-th  tooltip="销售数目">销售数目</mu-th>
  ...
  export default {
    ...
    methods: {
      ...
      addressChange (value, index) {
        ...
        this.sell.place = this.addressCity
      },
      checkAlreadySet(){
        if(this.sell.pid >= 0 && this.sell.time != '') {
          this.sell.num = parseInt(this.sell.num)
          this.$http.post(this.apiurl+'/sell/checkset',this.sell)
            .then((res) => {
              if(res.data === null) {
                this.errorText = ''
              }else {
      ...
      getSells() {
        this.$http.get(this.apiurl + '/sell/count')
          .then((res) => {
            if(res.data!== null) {
              this.page.total = res.data
            }
          })
        this.$http.post(this.apiurl +'/sell/get?current=' + this.page.current + '&pagesize='+this.page.pagesize)
          .then((res) => {
            if (res.data !== null) {
              this.sells = res.data
    ...
    computed: {
      disabled:function () {
        ...
        if(this.sell.place.replace(/(^s*)|(s*$)/g, "").length === 0) {
          return true
        }
        if(this.sell.time.replace(/(^s*)|(s*$)/g, "").length === 0) {
          return true
        }
        if(this.sell.num <= 0){
          return true
        }
        if(this.errorText.replace(/(^s*)|(s*$)/g, "").length !== 0) {
          return true
        }
        return false
      },
